...
import { getUrls, addUrl, getProduct } from "./action";
...
import { differenceInMinutes, endOfDay, format, subDays } from "date-fns";

export default function Dashboard() {
  const [state, action] = useFormState(addUrl, null);
  const [url, setUrl] = useState<AirbnbSearchParams>();
  const [user, setUser] = useState<User | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [product, setProduct] = useState<Product | null>(null);
  const [urlObjects, setUrls] = useState<
    { url: string; listingUrls: string }[] | null
  >(null);
  ...
  function Urls() {
      ...
      <div className="flex gap-2">
        {urlObjects?.map((urlObject, i) => {
          ...
          return (
            <div
              key={i}
              className="rounded-lg p-10 space-y-3 text-sx break-words  border-4 size-1/3 "
            >
    ...
    function AddNewUrl() {
      return (
        <div className="rounded-lg p-10 space-y-4 border-dotted border-4 size-1/3">
          <h1>
